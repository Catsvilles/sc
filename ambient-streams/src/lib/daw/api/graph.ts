...
import type { Clip } from "$lib/daw/api/timeline";
...
export interface SourceScheduler {
  playClip(clip: Clip, when?: number, end?: number, offset?: number): void;
  stop(): void;
}
...
export abstract class Source<P extends Player> extends Effect {
  ...
  abstract createScheduler(player: P, outNode: GainNode): SourceScheduler;
}
