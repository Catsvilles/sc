...
<div
  class="flex flex-row items-center m-2 p-4 border-b-2 border-[var(--navbar-color)] h-36"
>
  {#if title}
    <div class="flex flex-row items-center gap-2 text-sm w-48 flex-none">
      {@html icon}
      {title}
    </div>
  {/if}
      ...
      >
        ...
        {:else}
          ...
          <a href={editLink} class="mt-1">
            <button
              class="rounded-full p-0.5 border-2 transition duration-700 uppercase text-sm {dangerous
                ? 'border-red-500 text-red-500 hover:bg-red-500 hover:text-black'
                : 'border-white text-white hover:bg-white hover:text-black'} min-w-[145px]"
            >
              {editButtonTitle}
            </button>
          </a>
