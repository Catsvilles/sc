<script lang="ts">
	...
	let controller: { innerElements: string | any[]; perPage: any; destroy: () => void; prev: () => void; next: () => void; goTo: (arg0: any) => void; currentSlide: number; }
...
<div class="w-full">
	...
	<ul>
		{#each {length: totalDots} as _, i}
		    <li on:click={() => go(i*currentPerPage)} class='{isDotActive(currentIndex, i) ? "active" : ""} active:bg-white'></li>
		{/each}
	</ul>
    {/if}
</div>
