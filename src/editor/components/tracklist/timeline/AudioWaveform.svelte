<script lang="ts">
  ...
  function getBuckets(buffer, NUMBER_OF_BUCKETS = 600) {   
		//NUMBER_OF_BUCKETS number of "bars" the waveform should have
...
<div class="audio-waveform" bind:clientWidth={width}>
  <!-- {#if decodedBuffer !== undefined}
    <Waveform peaks={getBuckets(decodedBuffer)} width={width} percent={0} barWidth={0} color="blue" progressColor="#5d80ea" height={40} />
  {/if} -->
