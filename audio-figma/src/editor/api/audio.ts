...
import { volume, pan, automationVol } from "@app/stores/effects";
...
import type { SourceScheduler } from "@api/graph";
    ...
    return {
      connect(destination: AudioParam | AudioNode, output = 0, input = 0) {       
        if (destination instanceof AudioParam) {
          ...
          panNode.connect(player.masterGain, output);
        } else {
          outNode.connect(panNode);
          panNode.connect(player.masterGain, output, input);
        }
      },
