<script lang="ts">
    ...
    import VideoInOutPointsTimelineDisplay from './Displays/VideoInOutPointsTimelineDisplay.svelte';
    import VideoPreviewAreaTimelineDisplay from './Displays/VideoPreviewAreaTimelineDisplay.svelte';
    import ScoringAreasTimelineDisplay from './Displays/ScoringAreasTimelineDisplay.svelte';
    import KeyframeTimelineDisplay from './Displays/KeyframeTimelineDisplay.svelte';
    ...
    const thumbs_padding = 1;

    async function GetThumbnails(project) {
        const video_url = project.src
        if (!video_url) {
            return;
        }
        ...
        const thumbnail_blobs = await getThumbnailsInParalell(
            ...
            (progress) => {
                ...
                console.log($createLoadingThumbnailsPercent, "$createLoadingThumbnailsPercent");
    ...
    onMount(async () => {
        ...
        $createWaveSurfer.load(video)
        GetThumbnails(video);
    });

    const updateWaveSurferCurrentTime = (time: number) => {
        if (!$createWaveSurfer) {
            return;
        }
    ...
    const getNumThumbnailsToDisplay = async () => {
        ...
        for (let i = 0; i < num_thumbnails_to_display; i++) {
            const timestamp =
                duration * (i / num_thumbnails_to_display);
    ...
    const onClickTimeline = (e) => {
        ...
        $createVideoPlayer.seekTo(percentClick * duration, !mouseDownOnTimeline);
    };
    ...
    $: {
        seekerProgressPercent = `${
            ($createVideoCurrentTime /  duration) * 100
        }%`;
    }
...
<button on:pointerdown={()=>{video.play()}}>play</button>
<button on:pointerdown={()=>{video.pause()}}>pause</button>
...
>
    <div class="lines">
        ...
        <div class="timestamp right">
            {ConvertDurationToNiceStringWithDecimal(duration)}
        </div>
    </div>
